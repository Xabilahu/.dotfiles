...
# Code taken from: https://github.com/dbazile/gaudible
...
def main():
    ...
    ap.add_argument(
        ...
        dest="sound_spec",
        action="append",
        help="registers a sound for a specific filter with format <filter-name>:<file-path> or use format <file-path> for everything",
    )
    ...
    if params.sound_spec:
        for spec in params.sound_spec:
            ...
            if not os.access(value, os.R_OK):
                ap.error(
                    "audio file %r cannot be read in sound spec %r" % (value, spec)
                )
                return
    ...
    filter_keys = tuple(
        sorted(set(params.filters if params.filters else FILTERS.keys()))
    )
    ...
    def on_message(_, msg):
        ...
        try:
            ...
            for filter_key in filter_keys:
                ...
                # Append type of notification if Geary
                if filter_key == "geary" and len(args) > 1:
                    origin = f"{origin}.{str(args[1])}"

                if (
                    filter_interface == interface
                    and filter_method == method
                    and filter_origin == origin
                ):

                    # Suppress udio if we're in 'do not disturb' mode
...
def subscribe_to_messages(bus, filter_keys):
    ...
    for k in filter_keys:
        ...
        if interface == "org.freedesktop.Notifications":
            rule = "type=method_call, interface=%s, member=%s, sender=%s" % (
                interface,
                method,
                interface,
            )
...
class AudioPlayer:
    ...
    def play(self, name):
        ...
        t = threading.Thread(
            target=self._play,
            args=[name],
            name="%s:%s" % (self.__class__.__name__, time.time()),
        )
        t.start()
    ...
    def _enforce_rate_limit(self):
        ...
        if now <= self._quiet_until:
            LOG.debug(
                "audioplayer: in quiet period for another %.2f seconds",
        ...
        LOG.debug(
            "audioplayer: setting quiet period: now=%.2f quiet_until=%.2f rate_ms=%.2f",
            now,
            self._quiet_until,
            self._rate_ms,
        )
